...
// ----------------------------------------------------
// Database Manager (JSON File with Atomicity)
// ----------------------------------------------------
interface DBState {
  companies: any[];
  users: any[];
...
function logActivity(userId: string, username: string, companyId: string, action: string, details: string) {
  const db = loadDB();
...
app.get("/api/companies", authMiddleware, (req: any, res) => {
...
app.post("/api/companies", authMiddleware, (req: any, res) => {
  if (req.user.role !== UserRole.SUPER_ADMIN) {
...
app.put("/api/users/:id", authMiddleware, (req: any, res) => {
  ...
  if (departmentId !== undefined) userToUpdate.departmentId = departmentId;
...
// 4. Departments
app.get("/api/departments", authMiddleware, (req: any, res) => {
  const db = loadDB();
  const list = db.departments.filter((d) => d.companyId === req.user.companyId);
  res.json(list);
});

app.post("/api/departments", authMiddleware, (req: any, res) => {
...
app.put("/api/departments/:id", authMiddleware, (req: any, res) => {
  if (req.user.role !== UserRole.MANAGER) {
  ...
  if (index === -1) return res.status(404).json({ error: "القسم غير موجود." });

  if (name) db.departments[index].name = name;
  if (description !== undefined) db.departments[index].description = description;
  if (managerId !== undefined) db.departments[index].managerId = managerId;

  saveDB(db);
  res.json(db.departments[index]);
});

app.delete("/api/departments/:id", authMiddleware, (req: any, res) => {
  if (req.user.role !== UserRole.MANAGER) {
...
app.put("/api/projects/:id", authMiddleware, (req: any, res) => {
  if (req.user.role !== UserRole.MANAGER) return res.status(403).json({ error: "غير مصرح." });
  const { id } = req.params;
  const db = loadDB();
  const index = db.projects.findIndex((p) => p.id === id && p.companyId === req.user.companyId);
  if (index === -1) return res.status(404).json({ error: "المشروع غير موجود." });

  const { name, description, departmentId, status, startDate, endDate, progress } = req.body;
  if (name) db.projects[index].name = name;
  if (description !== undefined) db.projects[index].description = description;
  if (departmentId !== undefined) db.projects[index].departmentId = departmentId;
  if (status) db.projects[index].status = status;
  if (startDate) db.projects[index].startDate = startDate;
  if (endDate) db.projects[index].endDate = endDate;
...
app.post("/api/tasks", authMiddleware, (req: any, res) => {
  ...
  const newTask = {
    ...
    attachments: [],
    comments: [],
    checklist: (req.body.checklist || []).map((text: string, i: number) => ({ id: `chk_${Date.now()}_${i}`, text, isCompleted: false })),
    tags: tags || [],
...
app.put("/api/tasks/:id", authMiddleware, (req: any, res) => {
  ...
  if (index === -1) return res.status(404).json({ error: "المهمة غير موجودة." });
  ...
  if (req.user.role === UserRole.EMPLOYEE) {
    ...
    if (completionPercentage !== undefined) task.completionPercentage = Number(completionPercentage);
    ...
    if (assignedEmployeeId) task.assignedEmployeeId = assignedEmployeeId;
    if (requiredFiles !== undefined) task.requiredFiles = requiredFiles;
...
app.post("/api/tasks/:id/comments", authMiddleware, (req: any, res) => {
  const { id } = req.params;
  const { text } = req.body;
  if (!text) return res.status(400).json({ error: "محتوى التعليق مطلوب." });
...
// 7. Daily Evaluations
app.get("/api/evaluations", authMiddleware, (req: any, res) => {
  const db = loadDB();
...
app.post("/api/evaluations", authMiddleware, (req: any, res) => {
  if (req.user.role !== UserRole.MANAGER && req.user.role !== UserRole.SUPERVISOR) {
...
app.get("/api/attendance", authMiddleware, (req: any, res) => {
  const db = loadDB();
...
app.post("/api/attendance/clock-in", authMiddleware, (req: any, res) => {
...
app.post("/api/attendance/clock-out", authMiddleware, (req: any, res) => {
  const todayStr = new Date().toISOString().split("T")[0]; // YYYY-MM-DD
  const timeStr = new Date().toTimeString().split(" ")[0]; // HH:MM:SS
...
app.post("/api/attendance/leave-request", authMiddleware, (req: any, res) => {
...
app.put("/api/files/:id/approve", authMiddleware, (req: any, res) => {
  if (req.user.role !== UserRole.MANAGER) {
...
// 11. Announcements
app.get("/api/announcements", authMiddleware, (req: any, res) => {
  const db = loadDB();
  const list = db.announcements.filter((a) => a.companyId === req.user.companyId);
  res.json(list);
});

app.post("/api/announcements", authMiddleware, (req: any, res) => {
  if (req.user.role !== UserRole.MANAGER) {
    return res.status(403).json({ error: "صلاحية نشر الإعلانات تقتصر على مدير الشركة." });
  }

  const { title, content, priority } = req.body;
  if (!title || !content) return res.status(400).json({ error: "العنوان ومحتوى الإعلان حقول مطلوبة." });
...
app.get("/api/activity-logs", authMiddleware, (req: any, res) => {
  const db = loadDB();
...
app.post("/api/gemini/generate-evaluation", authMiddleware, async (req: any, res) => {
...
app.post("/api/gemini/generate-report", authMiddleware, async (req: any, res) => {
